...
class ShortcutTest < ActiveSupport::TestCase
  ...
  test "empty path is bad" do
    shortcut = FactoryGirl.build(:shortcut, :shortcut => '', :url => 'http://', :user => @user)
  ...
  test "find_by_path" do
    shortcut = FactoryGirl.create(:shortcut, :shortcut => 'j', :url => 'http://jazz', :user => @user)
    shortcut = FactoryGirl.create(:shortcut, :shortcut => 'j/%s', :url => 'http://jazz/workitem/%s', :user => @user)
  ...
  test "find_by_path escapes spaces in the wildcard substituted input" do
    shortcut = FactoryGirl.create(:shortcut, :shortcut => 'ml/%s', :url => 'http://people.lan.flt/mailing_lists/members/%s', :user => @user)
  ...
  test "find_by_path picks the best match" do
    shortcut = FactoryGirl.create(:shortcut, :shortcut => 'svn/%s', :url => 'http://svn/%s', :user => @user)
    shortcut = FactoryGirl.create(:shortcut, :shortcut => 'vipersvn/%s', :url => 'http://svn/viper/%s', :user => @user)
  ...
  test "find_by_path does not double escape" do
    ...
    shortcut = FactoryGirl.create(:shortcut, :shortcut => 'dashboard', :url => url, :user => @user)
    assert_finds_url(url, 'dashboard', @user)
  end

  test "shortcuts must not contain spaces" do
    shortcut = FactoryGirl.build(:shortcut, :shortcut => 'j %s', :url => 'http://jazz/%s', :user => @user)
  ...
  test "shortcuts must have at least one character and one slash before a wildcard" do
    shortcut = FactoryGirl.build(:shortcut, :shortcut => 'wi%s', :url => 'http://jazz/%s', :user => @user)
    assert_false shortcut.valid?

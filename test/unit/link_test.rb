...
class LinkTest < ActiveSupport::TestCase
  ...
  test "empty path is bad" do
    link = FactoryGirl.build(:link, :shortcut => '', :url => 'http://', :user => @user)
  ...
  test "find_by_path" do
    link = FactoryGirl.create(:link, :shortcut => 'j', :url => 'http://jazz', :user => @user)
    link = FactoryGirl.create(:link, :shortcut => 'j/%s', :url => 'http://jazz/workitem/%s', :user => @user)
  ...
  test "find_by_path escapes spaces in the wildcard substituted input" do
    link = FactoryGirl.create(:link, :shortcut => 'ml/%s', :url => 'http://people.lan.flt/mailing_lists/members/%s', :user => @user)
  ...
  test "find_by_path picks the best match" do
    link = FactoryGirl.create(:link, :shortcut => 'svn/%s', :url => 'http://svn/%s', :user => @user)
    link = FactoryGirl.create(:link, :shortcut => 'vipersvn/%s', :url => 'http://svn/viper/%s', :user => @user)
  ...
  test "find_by_path does not double escape" do
    ...
    link = FactoryGirl.create(:link, :shortcut => 'dashboard', :url => url, :user => @user)
    assert_finds_url(url, 'dashboard', @user)
  end

  test "shortcuts must not contain spaces" do
    link = FactoryGirl.build(:link, :shortcut => 'j %s', :url => 'http://jazz/%s', :user => @user)
  ...
  test "shortcuts must have at least one character and one slash before a wildcard" do
    link = FactoryGirl.build(:link, :shortcut => 'wi%s', :url => 'http://jazz/%s', :user => @user)
    assert_false link.valid?
